...
nSam = int(filsize/nChans/4);
data = np.fromfile(fname, dtype=np.float32, count=-1, offset=0);
...
fhead = {b'telescope_id': b'66',    # DSA?
  ...
  b'nbits': str(32).encode(),
  b'source_name': 'source'.encode(),
  b'data_type': b'1',   # ???
  b'nchans': str(384*16).encode(),
  b'machine_id': b'99', # ??
  b'tsamp': str(tbin*4*8.192e-6).encode(),
...
np.reshape(alldata,(alldata.shape[0]*alldata.shape[1]),order='F').astype('float32').tofile(outfile);
