...
void beamformer(char *input, float *wr, float *wi, int *output, int nChans, int nAnts, int nTimes, int nPols) {
	...
	for(int nTime=0;nTime<nTimes;nTime++){
		for(int nChan=0;nChan<48;nChan++){
			for(int i=0;i<8;i++){
				...
				output[(nTime*nChans+nChan*8+i)*nPols*2    ] = (int)(rx);
				output[(nTime*nChans+nChan*8+i)*nPols*2 + 1] = (int)(ix);
				output[(nTime*nChans+nChan*8+i)*nPols*2 + 2] = (int)(ry);
				output[(nTime*nChans+nChan*8+i)*nPols*2 + 3] = (int)(iy);
			}
		}
	}
}
...
int main (int argc, char *argv[]) {
	...
	int * output = (int *)malloc(sizeof(int)*nChans*nTimes*nPols*2);
