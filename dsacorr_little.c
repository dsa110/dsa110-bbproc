...
int main (int argc, char *argv[]) {
	...
	// for advancing data
	int startSam = (int)(480/fInt);
	int endSam = startSam + (int)(100/fInt);
	for (int dti=0;dti<startSam;dti++) {
	  printf("Advanced %d of %d\n",dti+1,startSam);
	  for (int ddti=0;ddti<nTimeSam;ddti++)
	    fread(input,nAnts*nChans*nTimes*nPols,1,ptr);
	}
	
	for(timeslo = startSam; timeslo < endSam; timeslo++) {
